	...
	private Collection<Pair<String, Method>> transformBasicComponentRequiredToLocalSink(FlowSpecification flow,
		...
		for (RequiredRole requiredRole : component.getRequiredRoles_InterfaceRequiringEntity()) {
			if (requiredRole instanceof OperationRequiredRole) {
				for (OperationSignature signature : ((OperationRequiredRole) requiredRole)
					...
					Interface scInterface = resolver.getInterface(
							((OperationRequiredRole) requiredRole).getRequiredInterface__OperationRequiredRole());
					edu.kit.ipd.sdq.composition.securityanalyses.coupling.structure.SourceCode.Method scMethod = resolver
							.getMethod(signature);
					if (flow.getSource().get(0) instanceof MethodIdentifying) {
						
						MethodIdentifying source = (MethodIdentifying)flow.getSource().get(0);
						
						
						if (!source.getMethod().getId().equals(scMethod.getId())) {
