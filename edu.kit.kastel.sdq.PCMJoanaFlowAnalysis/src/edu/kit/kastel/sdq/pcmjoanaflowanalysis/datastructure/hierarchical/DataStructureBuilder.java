...
public class DataStructureBuilder {
	...
	public SystemRepresentation buildRepresentationsForSystem(System system,
		...
		fillAllFlowBasicComponentsWithAnnotations(basicComponents);
...
//		systemRepresentation.printRepresentation();
	...
	private void fillFlowBasicComponentWithAnnotations(FlowBasicComponent component) {
		Collection<GenericTargetStringContentAnnotation> annotations = ModelsSubtypeResolver
				.filterGenericTargetStringContentAnnotations(currentWorkingAnnotations);

		for (GenericTargetStringContentAnnotation annotation : annotations) {
			if (annotation.getTarget() instanceof RepositoryComponent && ((RepositoryComponent) annotation.getTarget())
					.getId().equals(component.getComponent().getId())) {
				component.setAnnotation(annotation.getAnnotationType(), annotation.getContent());
			}
		}
	}
	
	private void fillAllFlowBasicComponentsWithAnnotations(Collection<FlowBasicComponent> components) {
		components.forEach(component -> fillFlowBasicComponentWithAnnotations(component));
	}
}
